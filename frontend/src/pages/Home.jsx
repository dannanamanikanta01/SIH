...
export default function Home() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Verification service error. Please try a clearer image.';
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <header className="app-header">
        ...
        {history.length > 0 && (
          <div className="history-toggle-row">
            ...
            >
              <History size={15} />
              <span>{showHistory ? 'Hide History' : `Past Verifications (${history.length})`}</span>
            </button>
          </div>
        )}
      </header>
            ...
            {history.slice(0, 5).map((item, i) => (
              ...
              >
                ...
                <div className="history-status-col">
                  <span className={`badge-pill ${item.status === 'COMPLIANT' ? 'pill-pass' : 'pill-fail'}`}>
